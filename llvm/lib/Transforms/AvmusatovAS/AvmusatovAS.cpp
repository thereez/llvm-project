...
void simplifyBinaryOperators(SmallVector<BinaryOperator*, 16> binOperators, size_t position) {
...
PreservedAnalyses AvmusatovAS::run(Function& F, FunctionAnalysisManager& AM) {
    ...
    for (auto& I : instructions(F)) {
        if (auto BO = dyn_cast<BinaryOperator>(&I)) {
            auto leftOpValue = dyn_cast<ConstantFP>(BO->getOperand(LEFT_OP))->getValue().convertToFloat();
            auto rightOpValue = dyn_cast<ConstantFP>(BO->getOperand(RIGHT_OP))->getValue().convertToFloat();
    ...
    simplifyBinaryOperators(leftWarkSet, LEFT_OP);
    simplifyBinaryOperators(rightWarkSet, RIGHT_OP);
